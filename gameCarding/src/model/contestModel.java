...
public class contestModel {
	...
	public ArrayList<contest> getList() {
		...
		try {
			...
			while(rs.next()) {
				a = new contest(rs.getInt(1),rs.getString(2),rs.getInt(3),rs.getString(4),rs.getString(5),rs.getInt(6),rs.getString(7));
				list.add(a);
			}
			ps.close();
			cn.close();
	...
	public int add() {
		...
		String sql = "INSERT INTO `contest`(`name`, `prize`, `money`) VALUES (?,?,?)";
		try {
			PreparedStatement ps = cn.prepareStatement(sql);
			ps.setString(1, c.getName());
			ps.setString(2, c.getPrize());
			ps.setInt(3, c.getMoney());
			kq=ps.executeUpdate();
	...
	public int update() {
		...
		String sql = "UPDATE `contest` SET `name`=?,`soluong`=?,`winner`=?,`prize`=?,`money`=?,`note`=? WHERE id=?";
		try {
			PreparedStatement ps = cn.prepareStatement(sql);
			ps.setString(1, c.getName());
			...
			ps.setString(4, c.getPrize());
			ps.setInt(5, c.getMoney());
